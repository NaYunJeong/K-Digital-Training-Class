...
--		개체(Entitiy) : 사람, 장소, 사물, 사건 등과 같이 독립적으로 존재하면서, 고유하게 식별이 가능한 단위
-- 		아무리 많은 개체가 있어도, 내가 원하는 '유일한' 개체를 발견할 수 있어야
--		테이블은 기본키를 지정하고, NULL값을 가져서는 안되며, 테이블 내에 오직 하나의 값만 존재해야한다는 조건.
...
--					=> 가장 많이 사용되는 제약 조건
--				: 테이블을 만들 때 가급적 값이 들어가게 해줫으면 좋겠음 ! ***
...
-- 3. Primary Key(기본키) : NULL값을 허용하지 않고 + 중복된 데이터를 허용하지 않음
...
-- *** 진짜 어려움
-- 4. Foreign Key (외래 키) : 다른 테이블의 특정 컬럼을 조회해서 동일한 데이터가 있는 경우에만 입력이 허용됨
--		참조받는 테이블은 참조하는 테이블의 PK나 Unique로 지정된 컬럼만을 FK로 지정할 수 있음
...
create table 테이블명(
	...
	constraint FK(제약조건)명 foreign key(컬럼명)
		...
		on delete set null || on delete cascade -- 둘 중 하나 입력
...
insert into major values(1005, '식품영양학과');
select * from major;
...
insert into student values(103, '홍길동', 1001);
insert into student values(104, '이성대', 1005);
insert into student values(105, '김성대', 3001); -- 오류 ! major 테이블에는 3001 코드 X

select * from student;

-- 어떤 특정학과가 없어지면 이 학생의 정보도 지워지게
delete from major where m_code = 1001;
-- major테이블에서 1001코드를 지우면, student테이블에서 1001코드를 가지고 있던 모든 학생의 정보가 지워짐
----------------------------------------------------------------------------------
-- 5. Check : 데이터의 값의 범위나 조건을 설정해서 조건에 해당하는 데이터만 입력을 허용
...
-- 6. Default : 아무런 데이터도 입력하지 않은 경우 지정한 데이터가 자동으로 입력됨
