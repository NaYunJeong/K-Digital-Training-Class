...
-- 그러면 이 NOT을 어디에 쓰는게 좋을까요? NOT IN ?(O) / IN NOT ?(X) 
...
-- '이'가 들어가는 과자들의 평균가격을 조회
select round(avg(s_price), 1) from jun03_snack where s_name like '%이%';
-- 크라운 과자거나, 콘치 ~ 인거 이름, 제조사, 가격 조회
select s_name, s_maker, s_price from jun03_snack where s_maker = '크라운' or s_name like '콘치%';
-- 과자의 이름이 '새콤달콤'을 포함하거나 '콘치'를 포함하는 과자 이름, 제조사, 가격 조회
select s_name, s_maker, s_price from jun03_snack where s_name like '%새콤달콤%' or s_name like '%콘치%';

-- like ~ in 구조는 아예 없음...!
select s_name, s_maker, s_price from jun03_snack where s_name like in ('%새콤달콤%', '%콘치%');  -- 에러 !

-- REGEXP_LIKE(컬럼명, '패턴1|패턴2|...')
-- 정규식(GEGEX)을 이용해서 문자를 찾는 방법인데...
...
select 1 id, '수학' subhect, 90 score from dual
	union
select 1 id, '수학' subhect, 90 score from dual
	union
select 1 id, '수학' subhect, 90 score from dual;
...
select 1 id, '수학' subject, 90 score from dual;
------------------------------------------------------------------------------------
-- 과자 데이터 중에서 회사가 크라운이고 가격이 1000원 이하이거나 3000원 이상인 과자의 전체 정보를 조회
-- 연산자 우선순위 (오른쪽이 최우선순위)
-- OR > AND > NOT > 비교연산자 > 괄호
select * from jun03_snack where s_maker = '크라운' and (s_price <= 1000 or s_price >= 3000);

select * from jun03_snack where s_price <= 1000
	union
...
select s_maker, round(avg(s_price), 1) 평균가, min(s_price) 최소가, max(s_price) 최고가 from jun03_snack
	group by s_maker;
...
-- 회사 이름, 회사별 과자 총 가격 합계와 모든 과자들의 가격 합계 조회
select s_maker, sum(s_price) 
	from jun03_snack
	group by rollup(s_maker);
	
-- s_maker가 null이면 '전체', 그게 아니면 s_maker 값 출력
select nvl2(s_maker, s_maker, '전체') s_maker, sum(s_price) 
	from jun03_snack 
	group by rollup(s_maker);
	
-- DECODE(컬럼명, 조건, 조건에 맞는 결과값, 조건에 맞지 않을 때 결과값)
select decode(s_maker, nullm '전체', s_maker) s_maker, sum(s_price) 
	from jun03_snack 
	group by rollup(s_maker);
-- decode 사용시 : 조건 자리에는 비교연산자(X), 산술연산자(O)
------------------------------------------------------------------------------------
-- Having : Group by 절에 의해 생성된 결과 중 원하는 조건에 부합하는 자료만 찾고자 할 때 사용
-- Having과 Where 모두 조건을 이용해서 원하는 조건에 맞는 데이터만 산출한다는 공통점
-- Having : 그룹의 결과를 제한
--		vs
...
-- 롯데 또는 해태 회사의 과자 평균 가격이 1800원 이상일 때 각 회사의 평균가격을 내림차순 정렬
