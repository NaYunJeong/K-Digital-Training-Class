...
--		ex) p_name varchar2(10 char) 이면 문자열을 최대 10글자까지 받겠다는 의미,
...
-- 		=> alt + x
create table person(
	p_name varchar2(10 char),	-- 최대 10자리
	p_age number(3),			-- 최대 3자리
...
--			Select, Insert, Update, Delete 등이 있음 (줄여서 C(insert)R(select_읽어오다)UD라고 함)
...
--			권한을 주고 회수하는데 사용
--			Grant, Revoke 등이 있음
--		제어어는 DBA(Database Administrator)의 업무라서 이정도만 보고 갑시다 ~
...
--		낙타체가 안통함(catAge) => 뱀체로(cat_age) 사용
-- 		BUT! 데이터는 대/소문자를 구별함
--			"a"와 "A"는 다르다.
